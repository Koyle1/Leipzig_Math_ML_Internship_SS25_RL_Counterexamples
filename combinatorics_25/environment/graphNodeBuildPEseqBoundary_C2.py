...
def calc_reward_nx(
    ...
    if not nx.is_connected(G):
        ...
        alpha += 0.5
        return -np.tanh(N_graph / 2.0) * alpha
    ...
    if np.isinf(dist_matrix).any():
        beta += 0.05
        return -np.tanh(N_graph / 2.0) * beta
    ...
    base_reward = - (proximity + partial_eig) - alpha * boundary

    # Soft transition to high reward if base_reward is good and fiedler is non-zero
    sigmoid_gain = soft_boundary_transition(base_reward, threshold=0.5) * soft_boundary_transition(fiedler_value, threshold=1e-6)
    reward = base_reward * (1 - sigmoid_gain) + sigmoid_gain * 10.0  # Soft blend

    # Optional save
    if reward > 5.0 and save_dir and N_graph > 4:
...
class GraphNodeBuildEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.use_surrogate and self.surrogate_model is not None:
            ...
            reward = calc_reward_nx(self.graph, fiedler_score = self.min_fiedler, end_of_note=(self.current_edge_idx == self.current_node))
